...
Const OPTION_UAGENTVERSION =				"UnpathcedAgentVersion"
...
Function CheckClient(ByRef options)
	...
	If errorCode = 0 Then
		configMgrLogPath = configMgrLogPath & "Logs\" & WScript.ScriptName & ".log"
		
		If configMgrLogPath <> g_logPathandName Then
		
			Err.Clear
			
			g_fso.MoveFile g_logPathandName, configMgrLogPath
			
			errorCode = Err.Number
			
			If errorCode = 0 Then
				g_logPathandName = configMgrLogPath
				WriteLogMsg MSG_CHECKCLIENT_MOVEDLOG & configMgrLogPath, 1, 1, 0
			Else
